...
$shaxsiy = "https://t.me/+ZEcQiRY_pRphZTdi";
$soat = date('H:i');
$sana = date("d.m.Y");
...
$array['inline_keyboard']["$i"][0]['text'] = "$check";
...
$solve = str_replace("solve","💳 Pul yechish",file_get_contents("tugma/solve.txt"));
$cabinet = str_replace("cabinet","📱 Kabinet",file_get_contents("tugma/cabinet.txt"));
...
$reklama = str_replace(["advertising","%first%","%last%","%id%","%username%","%botname%","%user%","%balance%","%refcount%","%currency%","%solve%","%hour%","%date%"], ["🤖 <b>Bot yaratish xizmati:</b> @AlijonovUz",$name,$last,$cid,$username,$bot,$user,$pul,$odam,$valyuta,$solv,$soat,$sana],file_get_contents("matn/advertising.txt"));
...
[['text'=>"💵 To'lov holati"],['text'=>"💳 To'lov tizimlari"]],
[['text'=>"$orqa"]]
]
]);
...
$refid = file_get_contents("step/$cid2.txt");
...
$pul = file_get_contents("pul/$cid3.txt");
$a = $pul + $taklif;
...
$minutes = str_pad(floor(($qoldi - $hours*60*60)/60), 2, '0', STR_PAD_LEFT);
	bot('SendMessage',[
	'chat_id'=>$admin,
	'text'=>"💵 <b>To'lov holati:

⏳  $kun kun, $hours soat, $minutes daqiqadan so'ng bot uyqu rejimiga o'tkaziladi.</b>",
'parse_mode'=>'html',
'reply_markup'=>json_encode([
	'inline_keyboard'=>[
